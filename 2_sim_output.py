...
def deleta():
  os.makedirs('temp')
  input_file = open('behaviors_surrogates.csv', 'r')
  output_file = open('temp/behaviors_surrogates_1.csv', 'w')
  ...
  input_file = open('temp/behaviors_surrogates_1.csv', 'r')
  output_file = open('temp/behaviors_surrogates_2.csv', 'w')
  data = csv.reader(input_file)
  writer = csv.writer(output_file)
  specials = ']'
  ...
  input_file = open('temp/behaviors_surrogates_2.csv', 'r')
  output_file = open('temp/behaviors_surrogates_3.csv', 'w')
  data = csv.reader(input_file)
  writer = csv.writer(output_file)
  specials = ' '
...
def markovM2(cadeia1,cadeia2):
  ...
  count2 = 1
  count3 = 1
  while (i<(len(cadeia1)-2)):
    if cadeia1[i]==1 and cadeia1[i+1]==1:
      count0+=1
    elif cadeia1[i]==1 and cadeia1[i+1]==-1:
      count1+=1
    elif cadeia1[i]==-1 and cadeia1[i+1]==1:
      count2+=1
    elif cadeia1[i]==-1 and cadeia1[i+1]==-1:
      count3+=1
    if cadeia1[i]==1 and cadeia1[i+1]==1 and cadeia2[i+2]==1:
      somaS0+=1
    elif cadeia1[i]==1 and cadeia1[i+1]==-1 and cadeia2[i+2]==-1:
      somaS1+=1
    elif cadeia1[i]==-1 and cadeia1[i+1]==1 and cadeia2[i+2]==1:
      somaS2+=1
    elif cadeia1[i]==-1 and cadeia1[i+1]==-1 and cadeia2[i+2]==1:
      somaS3+=1
    i+=1
  return [somaS0/count0,1-(somaS1/count1),somaS2/count2,somaS3/count3]


def main():
  deleta()
  transpose_csv('temp/behaviors_surrogates_3.csv', 'temp/behaviors_surrogates_4.csv')

  filename = "temp/behaviors_surrogates_4.csv"  # Change this to your CSV file name
  columns = convert_columns(read_csv_columns(filename))

  output_file = open('sim_output1' + '.csv', 'w', newline='')
  ...
  i = 0
  while (i<len(columns)):
    comportamento = columns[i][0]
    cadeia = columns[i][3]
    tipo = columns[i][2]
    proporcao = mean_response(columns[i][4:len(columns[0])])
    acuracia = accuracy_response(columns[i-1][4:len(columns[0])],columns[i][4:len(columns[0])])
  ...
  output_file = open('sim_output2' + '.csv', 'w', newline='')
  output_file.write('comportamento')
  output_file.write(' \t')
  output_file.write('cadeia')
  output_file.write(' \t')
  ...
  output_file = open('sim_output3' + '.csv', 'w', newline='')
  output_file.write('comportamento')
  output_file.write(' \t')
  output_file.write('cadeia')
  output_file.write(' \t')
  ...
  while (i<len(columns)-1):
    ...
    i+=2
  output_file.close()
  os.remove('temp/behaviors_surrogates_1.csv')
  os.remove('temp/behaviors_surrogates_2.csv')
  os.remove('temp/behaviors_surrogates_3.csv')
  os.remove('temp/behaviors_surrogates_4.csv')
  os.rmdir('temp')
main ()